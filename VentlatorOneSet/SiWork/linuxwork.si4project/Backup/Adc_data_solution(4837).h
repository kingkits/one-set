...
} ENUM_EEPROM_PRESS_SENSOR_DATA_TYPES;
...
typedef struct __ST_EEPROM_OXYGEN_DATA
{
    ...
    // base
    unsigned short int base;       //2      5-6
    // P_step
    unsigned int P_step;     //4     7-10
    // XOR
...
typedef struct __EEPROM_LIST_DATA
{
    //       name        lenth locate
    unsigned int head;       //4      0-3
    unsigned short int lenth;      //2        4

    EEPROM_LIST_POINT data[MAX_EEPROM_LIST_DATA_NUM];  //300 bytes = 6 * 50
    unsigned int sum_dat;
    unsigned char xor_dat;
} EEPROM_LIST_DATA;	// total 300+4+2+4+1
