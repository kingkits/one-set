...
__weak uint8_t check_breath_inspire_start(void)
{
...
// 检测患者吸气开始,此操作只在临近吸气时才可以调用，如果气道压力低于阈值（默认为0，不可以大于0），则界定为吸气开始
__weak uint8_t check_CPAP_breath_inspire_start(void)
{
    //如果吸气压力低于阈值，则吸气开始 ()
...
// 是否满足开始检测呼气的条件
__weak uint8_t check_breath_inspire_period(void)
{
    // 满足最小吸气时间 0.2秒
...
__weak uint8_t is_expire_flow_gate_begin(void)
{
    int32_t expire_flow;
    int32_t gate_flow;
    // 得到口端的气体流速
...
// 是否满足开始检测自主吸气的条件
__weak uint8_t check_breath_expire_period(void)
{
    // 满足最小呼气时间 0.2秒
    if(breath_get_time_interval() < 200)
        return EM_BREATH_NOT_FINISHED_SIGNAL; // 状态未触发
...
// 返回值： = 0 未检测到呼气信号
//          = 1 检测到有主动呼气信号
__weak uint8_t check_breath_expire_start(void)
{
...
__weak void breath_ST_inspire_monitor_status(void)
{
    // 如果吸气时间超过限定值则呼气开始
...
__weak uint8_t ST_check_breath_inspire_period(void)
{
    // 满足最小吸气时间 0.2秒
    if(breath_get_time_interval() < 200)
    {
...
__weak uint8_t st_check_breath_expire_period(void)
{
    // 满足最小呼气时间 0.2秒
    if(breath_get_time_interval() < 200)
        return EM_BREATH_NOT_FINISHED_SIGNAL; // 状态未触发
...
__weak void st_check_breath_inspire_period(void)
{
    // 吸气过程
    // 如果患者有主动呼气，则转换到呼气开始
    // 如果流量减少到初始流量的阈值，则呼气开始
...
__weak uint8_t PCV_check_breath_expire_period(void)
{
    // 满足最小呼气时间
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
...
//查看是否到了呼气结束
__weak uint8_t pcv_monitor_expire_end(void)
{
