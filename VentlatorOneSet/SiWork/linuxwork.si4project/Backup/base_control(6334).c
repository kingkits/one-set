...
{
    set_PID_blower_control_status(EM_BREATH_EXPIRE_SUPPORT);
...
int16_t *get_speed_main_control_data(void)
{
    ...
    case EM_BREATH_INSPIRE_SUPPORT: // 吸气时的补偿状态 Spontaneous Psupport
        return get_speed_control_data(ENUM_PID_BLOWER_CONTROL_PSUPPORT_ADJUST_DATA);
    case EM_BREATH_EXPIRE_SUPPORT:  // 呼气时的补偿状态 Spontaneous 辅助呼气
...
void main_control_fast_for_press(void)
{
    ...
    case EM_BREATH_EXPIRE_SUPPORT:	// 呼气时的补偿状态的C-flex Spontaneous 辅助呼气
...
{
    if(is_rap_actived()) return; // for rap control
    //变量定义
...
uint16_t get_blower_speed_for_press(int16_t press)
{
    ...
    //	uint32_t speed;
    index = press / 10;
    if(index > MAX_BLOWER_PRESS)index = MAX_BLOWER_PRESS;
...
void set_peep_press(uint16_t press)
{
    int index;
    index = press / 10;
    if(index > MAX_PEEP_PRESS)index = MAX_PEEP_PRESS;
...
void Blower_start_flow_compensate_for_inspire(void)
{
    ...
    //if(!is_main_base_control_flag_set(EM_I_COMPLETED_FLAGS))
    {
        ...
        // 设置当前工作的状态标记
        //set_PID_blower_control_status(EM_BREATH_INSPIRE_SUPPORT);
...
void Blower_start_flow_compensate_for_expire(void)
{
...
__weak uint16_t get_peep_value_with_press_and_flow(int16_t press, int32_t flow)
{
    ...
    // 可能需要控制最大值
    if(peep_val < PEEP_VALVE_MIN_SET_VALUE) peep_val =  PEEP_VALVE_MIN_SET_VALUE;
    if(peep_val > PEEP_VALVE_MAX_SET_VALUE) peep_val  = PEEP_VALVE_MAX_SET_VALUE;
...
__weak uint16_t get_blower_speed_for_press_and_flow(int16_t press, int32_t flow)
{
    ...
    // 检测是否溢出
    if(blower_val <  5000) blower_val =  5000;
    if(blower_val > 40000) blower_val = 40000;
