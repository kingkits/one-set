...
void prepare_pcv_first_time(void)
{
    ...
    pcv_control_dat.exhale_start_gate      = ms_1_count;
...
void prepare_PCV_control_data(void)
{
    ...
    // 基础数据-- 涡轮速度
    pcv_control_dat.low_speed = get_blower_speed_for_press_and_flow(main_control_data.epap, CONST_EXHALE_FLOW);
    pcv_control_dat.high_speed = get_blower_speed_for_press_and_flow(main_control_data.ipap, CONST_EXHALE_FLOW);
    pcv_control_dat.d_speed = (float) pcv_control_dat.high_speed - pcv_control_dat.low_speed;
    // 基础数据-- 呼气阀数据
    pcv_control_dat.peep_val = get_peep_value_with_press_and_flow(main_control_data.epap, CONST_EXHALE_FLOW);
...
void adjust_blower_speed_for_pcv()
{
    ...
    if(pcv_control_dat.high_speed <  5000)
    {
        pcv_control_dat.high_speed =  5000;
    }
    if(pcv_control_dat.high_speed > 40000)
    {
        pcv_control_dat.high_speed = 40000;
    }
    ...
    if(pcv_control_dat.low_speed <	5000)
    {
        pcv_control_dat.low_speed =  5000;
    }
