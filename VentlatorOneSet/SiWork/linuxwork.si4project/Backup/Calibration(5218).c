...
#define OXYGEN_SENSOR_BASE   760
#define OXYGEN_SENSOR_STEP   0.574
...
void load_default_calibration_data(void)
{
    ...
    sensor_data.O2_base               = OXYGEN_SENSOR_BASE;
    sensor_data.O2_step               = OXYGEN_SENSOR_STEP;

    //	sensor_data.e_l_flow_data_len     = DEFAULT_EXHALE_LOW_FLOW_DATA_LENTH;
    //	sensor_data.e_l_flow_real_val_ptr = expire_flow_val_data;
    //	sensor_data.e_l_flow_sensor_ptr   = expire_flow_sensor_data;
    //
    ...
    //	sensor_data.e_h_flow_sensor_ptr   = expire_flow_high_data;
    //
    //	sensor_data.O2_flow_data_len      = DEFAULT_OXYGEN_FLOW_DATA_LENTH;
    //	sensor_data.O2_flow_real_val_ptr  = Oxygen_flow_val_data;
    //	sensor_data.O2_flow_sensor_ptr    = Oxygen_flow_sensor_data;
    load_all_valve_default_data();
    // 读取铁电中保存的数据       // 检验是否读取成功
    // 如果成功则覆盖，否则使用缺省数据
...
uint8_t load_spi_ram_data(uint8_t types)
{
    return 0;
}
