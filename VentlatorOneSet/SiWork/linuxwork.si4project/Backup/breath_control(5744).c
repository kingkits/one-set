...
__weak void ST_set_flow_e_gate(int limited_flow)
{
    ...
    if(st_ST_mode_control_data.flow_e_gate < limited_flow) st_ST_mode_control_data.flow_e_gate = limited_flow;
}
...
__weak void breath_st_mode(void)
{
    ...
    case EM_PATIENT_T_EXPIRE_PERIOD: // T模式呼气过程
        ...
        if(is_rap_enable() && (rap_run_flag == 0))
        {
            ...
            // 如果延时不到，则返回
            if(breath_get_time_interval() < 150) break;
...
#else
    if(ms_1_count > st_PCV_mode_control_data.T_next_breath_start_gate) return EM_TRUE;
    return EM_FALSE;
#endif
}
...
__weak void prepare_cough_control_data(void)
{
    ...
    if(t1 < VAL_PRESS_INCREASE_STEP_TIME * 2) t1 = VAL_PRESS_INCREASE_STEP_TIME * 2; // 防止除0，保障最小上升时间  100ms
...
#endif
    // 目标潮气量
    if(main_control_data.cough_start_vt < 300) main_control_data.cough_start_vt = 300;
...
__weak void force_start_vacuum_pump(void)
{
    ...
    if(is_vacuum_blower_work())return; //if(base_board_status.status.cough_system_ready) return;
    // 启动真空泵
...
{
    if(!ui_set_data.change_mode_flag) return 0;
    ui_set_data.change_mode_flag      = 0;
