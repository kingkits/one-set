...
extern TSI_FLOW_SENSOR_DATA tsi_flow_sensor_tmp;
extern ST_INSERT_DATA tsi_flow_sensor_T_data;
...
void Tsi840205_Get_Flow(void)
{
    ...
    //	if(!tsi_param_flag)
    {
        ...
        utmp = (uint32_t) tsi_flow_sensor_tmp.Vt + 10;	// 测量电压比计算电压高10mv左右
        // 转换成温度 °C
        tsi_flow_sensor_tmp.T = (int32_t)get_insert_dec_data(utmp, &tsi_flow_sensor_T_data);
        // 由Tcorr 修正T=T+Tcorr
        tsi_flow_sensor_tmp.T += tsi_flow_para.Tcorr;

        // 得到流量电压值 Vf： flow voltage （mV）
        tsi_flow_sensor_tmp.Vf = trans_adc3_data_to_volt(adc_voltage.adc_tsi_V);
        tsi_flow_sensor_tmp.Vf += tsi_flow_sensor_tmp.Vf; //*= 2;	// 转接板电压分压，恢复原始电压
        // 计算标准参考值
        // vb = (Vf + Z) / S
        // TempCompFactor = K0 + K1 *Vb + K 2 *T + K3 *T*T + K4 *T*T*T
        // V f Std = (V f + Z) ?TempCompFactor ? Z
        ftmp0 = tsi_flow_sensor_tmp.T * 0.001 - 20;
        tsi_flow_sensor_tmp.Vf = tsi_flow_sensor_tmp.Vf * (float32_t)0.001;	// voltage V
        vb = (tsi_flow_sensor_tmp.Vf + tsi_flow_para.Z) / tsi_flow_para.S;
        ftmp1 = ftmp0 * ftmp0;
        ftmp2 = ftmp0 * ftmp1;
        ...
        tsi_flow_sensor_tmp.Vstd = (tsi_flow_sensor_tmp.Vf + tsi_flow_para.Z) * ftmp3 - \
                                   tsi_flow_para.Z;

        // 获取计算系数Ai、Bi、Ci
        Tsi840205_CalcCoeff(tsi_flow_sensor_tmp.Vstd, &Ai, &Bi, &Ci);
        ...
        tsi_flow_sensor_tmp.Q = Ai;
        tsi_flow_sensor_tmp.Q += Bi * vv;
        tsi_flow_sensor_tmp.Q += Ci * vvvvv;
        // 转换成整形（ml/min）并返回
        tsi_flow_sensor_tmp.Q = tsi_flow_sensor_tmp.Q * (float32_t)1000; // 由L/Min --> mL/Min
#if 0
        flow_temp[num++] = (long)tsi_flow_sensor_tmp.Q;
        if(num == 12)
        {
            num = 0;
...
#else
        adc_convert_data.flow_1_val = (int32_t)tsi_flow_sensor_tmp.Q;
#endif
    }
}
