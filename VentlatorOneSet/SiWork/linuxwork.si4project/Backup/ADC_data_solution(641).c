 ...
 * @param  src_ptr          [description]
 * @param  tar_ptr          [description]
 ...
 * @param  press_data_ptr         [description]
 * @return                        [description]
 */
...
unsigned char *package_eeprom_list_data(uint8_t list_type)
{
    ...
    *ch_ptr = data_ptr->lenth;
    ...
    //如果长度不符合要求，则返回
    if((data_ptr->lenth >= 50) || (data_ptr->lenth < 5))
        goto PREPARE_LIST_DATA_ERROR;
    ...
    *ch_ptr = list_type & 0xf;
    ...
    // count lenth and append xor byte
...
{
    if(list_type > EM_LIST_DATA_VALVE_BACKFLOW_DATA)
        return null;
    else
...
/*
 "字节1：		  传感器位置
 字节2： 		 校准点数
 字节(3-5):	   流量				  (3bytes)	 (第1个点）
 字节(6-7):	   对应传感器值  (2bytes) （第1个点）
 字节(8-10):	  流量				 (3bytes) （第2个点）
 字节(11-12):  对应传感器值  (2bytes) （第2个点）
 */
