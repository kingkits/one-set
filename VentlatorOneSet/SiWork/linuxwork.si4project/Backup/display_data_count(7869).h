...
typedef struct __ST_DISPLAY_DATA
{
    ...
    //峰值流速  // 用于肺功能监测
    int32_t inspire_peak_flow;
    int32_t expire_peak_flow;
...
typedef struct __ST_DISPLAY_TEMP_DATA
{
    ...
    // x个周期的吸呼气时间
    uint32_t inspire_time[MAX_BREATH_COUNT];
    uint32_t expire_time[MAX_BREATH_COUNT];
    ...
    // PPFFVV for test!!!!
    //int16_t inspire_Press; // 当前吸气口压力
    //int16_t expire_Press; // 当前吸气口压力

    //int32_t inspire_Flow;  // 当前吸气口流速
    //int32_t expire_Flow;  // 当前呼气口流速
    int32_t inspire_peak_flow;
    int32_t expire_peak_flow;

    int16_t inspire_Volume;// 当前计算的吸气潮气量
    int16_t expire_Volume;// 当前计算的呼气潮气量


    uint8_t trigger_inspire_flag;
    uint8_t trigger_expire_flag;
