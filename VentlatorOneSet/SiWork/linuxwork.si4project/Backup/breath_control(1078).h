...
typedef enum _ENUM_MAIN_WORK_STATUS_TYPE_DEFINES
{
    ...
    EM_BREATH_INSPIRE_SUPPORT, // 吸气时的补偿状态 辅助吸气      ，未确定
    EM_BREATH_EXPIRE_SUPPORT // 呼气时的补偿状态 辅助呼气    ，main_control_data.c_flex_level
...
typedef struct _ST_MAIN_CONTROL_DATA
{
    ...
    //T mode breath inspire time T 模式的吸气时间
    uint32_t T_inspire_time; // (1ms)
...
typedef struct __st_ST_mode_control_data_DEFINES
{
    ...
    uint32_t T_expire_start_gate;  // 下一呼气开始时刻
...
uint8_t  is_ST_breath_expire_start_time(void);
void     to_default_breath_status(void);
uint16_t pcv_get_inspire_end_time(void);
int16_t  get_PCV_T_target_press(void);
uint16_t PCV_get_expire_end_time(void);

uint16_t ST_get_expire_end_time(void);
