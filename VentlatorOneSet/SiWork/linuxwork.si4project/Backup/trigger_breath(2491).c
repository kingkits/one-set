...
void trigger_set_current_status(void)
{
    ...
    case EM_TRIGGER_TYPE_STV_INS_FINISHED:  // = 6, // ST 检测吸气结束
        trigger_set_stv_flow_for_inhale_finished(display_count_data.Flow);
        break;
    case EM_TRIGGER_TYPE_STV_EXP_FINISHED:  // = 7, // ST 检测呼气结束
        trigger_set_stv_flow_for_exhale_finished(display_count_data.Flow);
        break;
    }
}
