 ...
 */
/*
	7-25 属于完美状态 这个流量包括呼气回路中的流量，
	总的泄漏过小会造成患者CO2中毒
	当患者口端泄漏超过30时，	     要考虑减小呼气回路中的预设流量
	26-60 需要仔细补偿
	60-80 已经超出了补偿能力，数据开始失真，出现逐渐加重的控制失调
	80-120 尝试做最坏的努力
	>120 任何努力都是徒劳的
*/
...
/**
 * [enable_volume_count description]
 * @method enable_volume_count
 */
...
/**
 * [disable_volume_count description]
 * @method disable_volume_count
 */
...
/**
 * [get_breath_trigger_flag description]
 * @method get_breath_trigger_flag
 * @return                         [description]
 */
uint8_t get_breath_trigger_flag(void)
{
    return display_count_data.trigger_flag;
}

/**
 * [breath_set_trigger_active_breath_type description]
 * @method breath_set_trigger_active_breath_type
 * @param  type                                  [description]
 */
void breath_set_trigger_active_breath_type(uint16_t type)
{
    if(type > EM_TRIGGER_EXHALE_IN_BREATH) display_count_data.trigger_flag = EM_TRIGGER_INHALE_INACTIVE_BREATH;
    else display_count_data.trigger_flag = type;
}
...
{
    if(display_temp_data.P_meancount > 300)
    {
...
/**
 * [display_get_Pmean description]
 * @method display_get_Pmean
 * @return                   [description]
 */
...
{
    if(display_temp_data.inhale_peak_flow < 10000)
        return 10000;
    else
 ...
 * @method reset_leak_count_data
 */
void reset_display_leak_count_data(void)
{
    display_temp_data.flow_i_sum = 0;
    display_temp_data.flow_e_sum = 0;
    display_temp_data.flow_count = 0;
...
/**
 * [calculate_defult_leak_flow_new description]
 * @method calculate_defult_leak_flow_new
 */
void calculate_defult_leak_flow_new(void)
{
    float flow_i;
    float flow_e;
    int32_t leak;
    ...
    flow_i = (float)display_temp_data.inhale_Flow;//get_ins_flow();
    flow_e = (float)display_temp_data.exhale_Flow;//get_ex_flow();
