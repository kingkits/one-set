...
#define EEPROM_PRESS_EXHALE_ADDRESS     0x20
#define EEPROM_PRESS_VACUUM_ADDRESS     0x30
...
uint8_t check_eeprom_buf(uint8_t *src_ptr, uint8_t *tar_ptr, int len)
{
    ...
    for(i = 0; i < len; i++)
    {
        if(*src_ptr != *tar_ptr) return EEPROM_OPERATION_FALSE;
        xor = *src_ptr;
...
uint8_t check_eeprom_oxygen_data(void)
{
    ...
    if(xor == 0) return EEPROM_OPERATION_SUCCEED;
    else         return EEPROM_OPERATION_FALSE;
}
...
uint8_t eeprom_save_oxygen_concentration_data(void)
{
    ...
    // read
    FM25V05_READ(EEPROM_O2_CONCENTRATION_ADDRESS, (uint8_t *)eeprom_temp_buf, ST_EEPROM_OXYGEN_DATA_LEN);
...
uint8_t eeprom_check_press_xor(UNION_EEPROM_PRESS_DATA *press_data_ptr)
{
    ...
    {
        if(x_dat) return EEPROM_OPERATION_FALSE;
        else return EEPROM_OPERATION_SUCCEED;
    }
...
uint8_t eeprom_load_press_data(uint8_t type)
{
    ...
    case EM_SENSOR_PRESS_INHALE:
        FM25V05_READ(EEPROM_PRESS_INHALE_ADDRESS, (uint8_t *)eeprom_press_data.bytes, ST_EEPROM_PRESS_DATA_LEN);
    ...
    case EM_SENSOR_PRESS_EXHALE:
        FM25V05_READ(EEPROM_PRESS_EXHALE_ADDRESS, (uint8_t *)eeprom_press_data.bytes, ST_EEPROM_PRESS_DATA_LEN);
        if(eeprom_check_press_xor((UNION_EEPROM_PRESS_DATA *) &eeprom_press_data) == EEPROM_OPERATION_FALSE)
            return EEPROM_OPERATION_FALSE; // data error need load default data
    ...
    case EM_SENSOR_PRESS_VACUUM:
        FM25V05_READ(EEPROM_PRESS_VACUUM_ADDRESS, (uint8_t *)eeprom_press_data.bytes, ST_EEPROM_PRESS_DATA_LEN);
        if(eeprom_check_press_xor((UNION_EEPROM_PRESS_DATA *) &eeprom_press_data) == EEPROM_OPERATION_FALSE)
            return EEPROM_OPERATION_FALSE; // data error need load default data
...
#else
        FM25V05_READ(EEPROM_PRESS_INHALE_ADDRESS, (uint8_t *)eeprom_temp_buf, ST_EEPROM_PRESS_DATA_LEN);
...
uint8_t eeprom_check_list_data(void)
{
    ...
    // head
    if(eeprom_list_data.head != EEPROM_LIST_DATA_HEAD) return EEPROM_OPERATION_FALSE;
    ...
    if(eeprom_list_data.sum_dat != sum_data) return EEPROM_OPERATION_FALSE;
...
uint8_t eeprom_load_list_data(uint8_t data_type)
{
    ...
    {
    case EM_EEPROM_LIST_FLOW_EX_LOW:
        FM25V05_READ(EEPROM_FLOW_EXHALE_LOW_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_FLOW_EX_HIGH:
        FM25V05_READ(EEPROM_FLOW_EXHALE_HIGH_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_FLOW_OXYGEN:
        FM25V05_READ(EEPROM_FLOW_OXYGEN_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_BLOWER_PRESS:
        FM25V05_READ(EEPROM_BLOWER_PRESS_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_BLOWER_SPEED:
        FM25V05_READ(EEPROM_BLOWER_SPEED_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_BLOWER_FLOW0:
        FM25V05_READ(EEPROM_BLOWER_FLOW0_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_VALVE_PEEP:
        FM25V05_READ(EEPROM_VALVE_PEEP_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_BLOWER_VACUUM:
        FM25V05_READ(EEPROM_BLOWER_VACUUM_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_VALVE_FLOWBACK:
        FM25V05_READ(EEPROM_VALVE_BACKFLOW_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;

    case EM_EEPROM_LIST_VALVE_OXYGEN:
        FM25V05_READ(EEPROM_VALVE_OXYGEN_ADDRESS, byte_ptr, EEPROM_LIST_DATA_LENTH);
        break;
...
void active_eeprom_list_data(uint8_t data_type)
{
    ...
    case EM_EEPROM_LIST_FLOW_EX_LOW:
        ...
        sensor_data.e_l_flow_sensor_ptr   = (uint32_t *) & (eeprom_flow_ex_low_data.cpu_managed_val);
    ...
    case EM_EEPROM_LIST_FLOW_EX_HIGH:
        ...
        sensor_data.e_h_flow_sensor_ptr   = (uint32_t *) & (eeprom_flow_ex_high_data.cpu_managed_val);
    ...
    case EM_EEPROM_LIST_FLOW_OXYGEN:
        ...
        sensor_data.O2_flow_sensor_ptr   = (uint32_t *) & (eeprom_flow_oxygen_data.cpu_managed_val);
...
void active_default_eeprom_list_data(uint8_t data_type)
{
    ...
    case EM_EEPROM_LIST_FLOW_EX_LOW:
        ...
        sensor_data.e_l_flow_sensor_ptr   = (uint32_t *)&expire_flow_sensor_data;

        // set adc data
        init_insert_data(&expire_low_flow_data,
                         sensor_data.e_l_flow_data_len,
                         sensor_data.e_l_flow_sensor_ptr,
    ...
    case EM_EEPROM_LIST_FLOW_EX_HIGH:
        ...
        sensor_data.e_h_flow_sensor_ptr   = (uint32_t *)&expire_flow_high_idx_data;

        // set adc data
        init_insert_data(&expire_high_flow_data,
                         sensor_data.e_h_flow_data_len,
                         sensor_data.e_h_flow_real_val_ptr,
                         sensor_data.e_h_flow_sensor_ptr,
                         ENUM_DATA_IS_INCREASE);
        break;
    ...
    case EM_EEPROM_LIST_FLOW_OXYGEN:
        ...
        sensor_data.O2_flow_sensor_ptr   = (uint32_t *)&Oxygen_flow_sensor_data;

        // set adc data
        init_insert_data(&Oxygen_flow_data,
                         sensor_data.O2_flow_data_len,
                         sensor_data.O2_flow_sensor_ptr,
...
/* **********************************
   ...
   **********************************
 */
void ui_set_oxygen_sensor_data(uint8_t *bufptr)
{
    ...
    eeprom_oxygen_data.data.head   = EEPROM_OXYGEN_C_DATA_HEAD;
    eeprom_oxygen_data.data.status = EEPROM_OPERATION_SUCCEED;
    ...
    {
        reset_oxygen_sensor_ack_status();
...
void ui_set_press_sensor_data(uint8_t *bufptr)
{
    ...
    eeprom_press_data.data.head   = EEPROM_PRESS_DATA_HEAD;
    eeprom_press_data.data.status = EEPROM_OPERATION_SUCCEED;
    switch(*(bufptr + 1))
    {
    case 0: // =0 吸气压力
    ...
    {
        reset_press_sensor_ack_status();
    }
    else
    {
        set_press_sensor_ack_status();
    }
