...
__weak void oxygen_adjust(void)
{
    ...
    //得到当前流量
    flow = (float64_t) get_inhale_flow();
...
void set_concentration_value(uint8_t val)
{
    ...
    {
        if(oxygen_mixer_ctrol.set_concentration > get_inhale_oxygen_concent() + 50.0)
        {
            oxygen_pid_refresh_mode(ENUM_PID_OXYGEN_CONCENTRATION_LOW_START);
        }
        else if(oxygen_mixer_ctrol.set_concentration < get_inhale_oxygen_concent() - 50.0)
        {
...
__weak void oxygen_pid_adjust(void)
{
    ...
    case ENUM_PID_OXYGEN_CONCENTRATION_LOW_START: // 从低于设置的氧浓度值开始
        if(oxygen_mixer_ctrol.set_concentration < get_inhale_oxygen_concent() + 40.0)
        {
            //oxygen_pid_refresh_mode(ENUM_PID_OXYGEN_MIXER_ACTIVED);
    ...
    //得到当前流量
    ftmp = (float64_t) get_inhale_flow();
    if(ftmp < 500)
        return; // 流量太小不调整

    // 得到上次补偿后的实际数值
    current_o2_concent = (float64_t)get_inhale_oxygen_concent();
