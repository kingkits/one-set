...
{
    leak_test_data.current_press = (get_inhale_press() + get_exhale_press()) / 2;
    leak_test_data.current_count_leak = flow_leak_compensation(leak_test_data.current_press);
}
void leak_test_count_base(void)
{
    leak_test_data.exp_flow_sum_data += (float64_t) get_exhale_flow();
    leak_test_data.ins_flow_sum_data += (float64_t) get_inhale_flow();
    leak_test_data.press_sum_data    += (float64_t)(get_inhale_press() + get_exhale_press()) / 2.0;
