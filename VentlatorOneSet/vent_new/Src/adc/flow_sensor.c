...
int32_t inhale_flow_val_buf[FLOW_DATA_BUFFER_LEN];
int inhale_flow_buffer_ptr;

/**
 *
 *
 * @method reset_inhale_flow_buffer
 */
void reset_inhale_flow_buffer(void)
{
    ...
    {
        inhale_flow_val_buf[i] = 0;
        inhale_flow_buffer_ptr = 0;
    }
}
 ...
 *
 *
 * @method insert_inhale_flow_data
 * @param  val                      [description]
 */
void insert_inhale_flow_data(int32_t val)
{
    inhale_flow_val_buf[inhale_flow_buffer_ptr] = val;
    inhale_flow_buffer_ptr ++;
    if(inhale_flow_buffer_ptr >= FLOW_DATA_BUFFER_LEN)
    {
        inhale_flow_buffer_ptr = 0;
    }
}
...
uint32_t get_inhale_flow_data(void)
{
    ...
    int32_t fmax, fmin;
    int32_t ftotal;
    fmax   = inhale_flow_val_buf[0];
    fmin   = inhale_flow_val_buf[0];
    ftotal = inhale_flow_val_buf[0];
    for(i = 1; i < FLOW_DATA_BUFFER_LEN; i++)
    {
        if(fmax < inhale_flow_val_buf[i]) fmax = inhale_flow_val_buf[i];
        if(fmin > inhale_flow_val_buf[i]) fmin = inhale_flow_val_buf[i];
        ftotal += inhale_flow_val_buf[i];
    }
