...
// 用于检测患者呼吸状态的返回标志
typedef enum __ENUM_BREATH_INHALE_END_TYPES
{
...
typedef enum __ENUM_PATIENT_STATUS
{
    ...
    EM_PATIENT_INHALE_DETECT,  // 监测吸气触发条件
    EM_PATIENT_INHALE_START,   // 吸气开始的时刻
    EM_PATIENT_INHALE_PERIOD,  // 吸气的过程中
    EM_PATIENT_EXHALE_DETECT,  // 监测呼气触发条件
    EM_PATIENT_BREATH_HOLD,    // 屏气过程（仅容控过程可能使用）
    EM_PATIENT_EXHALE_START,   // 吸气转呼气的时刻
    EM_PATIENT_EXHALE_PERIOD,  // 呼气过程
    ...
    // 机械通气过程
    /* *******************************************************************/
    EM_PATIENT_T_INHALE_START,
    EM_PATIENT_T_INHALE_PERIOD,
    EM_PATIENT_T_EXHALE_START,
    EM_PATIENT_T_EXHALE_DETECT,
    EM_PATIENT_T_EXHALE_PERIOD,
    EM_PATIENT_T_INHALE_DETECT,
    /* *******************************************************************/
    // 状态结束标记
    /* *******************************************************************/
...
// 检测患者吸气开始
uint8_t check_breath_inhale_start(void);

// 检测患者呼气开始
uint8_t check_breath_exhale_start(void);

// 初始化函数
void init_patient_status(void);
