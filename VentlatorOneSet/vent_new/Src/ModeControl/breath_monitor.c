...
__weak uint8_t check_breath_inhale_start(void)
{
    ...
    if(is_Pressure_st_gate(main_control_data.epap - 20))
    {
        ...
        {
            breath_set_inhale_trigger_flag();
...
// 检测患者吸气开始,此操作只在临近吸气时才可以调用，如果气道压力低于阈值（默认为0，不可以大于0），则界定为吸气开始
__weak uint8_t check_CPAP_breath_inhale_start(void)
{
    //如果吸气压力低于阈值，则吸气开始 ()
...
// 是否满足开始检测呼气的条件
__weak uint8_t check_breath_inhale_period(void)
{
    // 满足最小吸气时间 0.2秒
...
__weak uint8_t is_exhale_flow_gate_begin(void)
{
    int32_t exhale_flow;
    int32_t gate_flow;
    // 得到口端的气体流速
    exhale_flow = breath_get_current_flow();
    ...
    // 取反，求绝对值
    exhale_flow = 0 - exhale_flow;

    // 计算 门限
    gate_flow = breath_get_exhale_peak_Flow() * main_control_data.gate_flow_limited_e / 100;

    if(is_Flow_st_gate(gate_flow)) //if(exhale_flow < gate_flow)
        return EM_TRUE;
...
// 是否满足开始检测自主吸气的条件
__weak uint8_t check_breath_exhale_period(void)
{
    // 满足最小呼气时间 0.2秒
    if(breath_get_time_interval() < 200)
        return EM_BREATH_NOT_FINISHED_SIGNAL; // 状态未触发
...
// 返回值： = 0 未检测到呼气信号
//          = 1 检测到有主动呼气信号
__weak uint8_t check_breath_exhale_start(void)
{
    // 如果呼气峰值流速超过阈值（这个阈值应该可以设置）
    if(is_Flow_st_gate(st_ST_mode_control_data.flow_e_gate))//if(breath_get_exhale_peak_Flow() > 3000)
    {
        return PATIENT_AUTONOMOUS_RESPIRATION;
    }
...
__weak void breath_ST_inhale_monitor_status(void)
{
    // 如果吸气时间超过限定值则呼气开始
    switch(check_breath_inhale_period())
    {
...
// 初始化函数
__weak void init_patient_status(void)
{
    global_patient_status = EM_PATIENT_NOT_WORK;
}

__weak void breath_monitor_CPAP(void)
{
    ...
    case EM_PATIENT_INHALE_PERIOD: // 吸气过程
        // 要等压力上升完成
        //Blower_start_flow_compensate_for_inhale();// 这个动作应该考虑延迟一小会，以避免误操作（怎么延迟？待确认，我考虑延迟0.1-0.3秒）
        // 吸气过程
        // 如果患者有主动呼气，则转换到呼气开始
...
__weak uint8_t ST_check_breath_inhale_period(void)
{
    // 满足最小吸气时间 0.2秒
    if(breath_get_time_interval() < 200)
    {
...
__weak uint8_t st_check_breath_exhale_period(void)
{
    // 满足最小呼气时间 0.2秒
    if(breath_get_time_interval() < 200)
        return EM_BREATH_NOT_FINISHED_SIGNAL; // 状态未触发
...
__weak void st_check_breath_inhale_period(void)
{
    // 吸气过程
    // 如果患者有主动呼气，则转换到呼气开始
    // 如果流量减少到初始流量的阈值，则呼气开始
    // 如果吸气时间超过限定值则呼气开始
    switch(ST_check_breath_inhale_period())
    {
...
__weak void breath_monitor_ST(void)
{
    ...
    case EM_PATIENT_EXHALE_PERIOD: // 呼气过程
        ...
        // 在呼气时，需要监测的内容
        switch(st_check_breath_exhale_period())
        {
        case EM_BREATH_NOT_FINISHED_SIGNAL: // 状态未触发
...
__weak uint8_t PCV_check_breath_exhale_period(void)
{
    // 满足最小呼气时间
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
...
//查看是否到了呼气结束
__weak uint8_t pcv_monitor_exhale_end(void)
{
    ...
    // 如果呼气超过15秒 则认为呼气结束
    if(breath_get_time_interval() > pcv_get_inhale_end_time())
    {
        return EM_TRUE;
    }
    // 否则未结束
...
__weak void breath_monitor_PCV(void)
{
    static int monitor_flag = 0;
    switch(global_patient_status)
    {
    case EM_PATIENT_PIPE_OUT: // 当管路脱落时，需要检测管路状态
        ...
        set_patient_status(EM_PATIENT_T_INHALE_START); // 强制进入吸气模式？
        break;
...
//
__weak void patient_status_monitor(void)
{
