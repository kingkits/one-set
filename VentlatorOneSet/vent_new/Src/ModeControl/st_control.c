...
void prepare_s_mode_target_press(int16_t epress, int16_t ipress)
{
    ...
    // get speed
    stv_control_dat.high_speed = get_blower_speed_for_press_and_flow(stv_control_dat.target_press, CONST_ST_EXHALE_FLOW);
}
...
void adjust_stv_speed(void)
{
    ...
    else stv_control_dat.err_blower_speed = 0; // 如果调整过程中出现了压力提前超过目标压力，则在另一调整过程中已经处理过了，这里就不需要再调整了
...
{
    if(flow < TRIGGER_INHALE_FLOW_GATE) return; // do nothing,只检测吸气过程，在吸气流量小于2000（2升每分钟）以后，认为吸气结束，
