...
void prepare_pcv_time(void)
{
    ...
    // 由呼吸频率决定，确定T触发时间
    // 这是正常的呼吸节奏，由吸气相开始！！！
    pcv_control_dat.breath_start_gate = ms_1_count + pcv_control_dat.one_breath_time;
    pcv_control_dat.exhale_start_gate = ms_1_count + main_control_data.T_inhale_time;

    // PCV在呼气相的3/4才打开吸气检测
    if(pcv_control_dat.one_breath_time > main_control_data.T_inhale_time + 500)
    {
        // 呼气相的3/4
        pcv_control_dat.T_exhale_3_4_gate = ((pcv_control_dat.one_breath_time - main_control_data.T_inhale_time) * 3 / 4) + pcv_control_dat.exhale_start_gate;
    }
    else
    {
        // 在呼气时间少于500ms时。要只保留200ms，在频率过高时，可以考虑不进行吸气触发的检测
