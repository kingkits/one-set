...
__weak uint8_t is_rap_time_over(uint8_t type)
{
    switch(type)
    {
    case EM_RAP_EXHALE_METHOD:
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
        return(system_direct_compare_tm1_be_tm2(ms_1_count, breath_rap_control_data.time_stamp + breath_rap_control_data.exhale_work_time));
#else
        if(ms_1_count - breath_rap_control_data.time_stamp > breath_rap_control_data.exhale_work_time)// TDB
        {
            return EM_TRUE;
        }
        else
...
void test_rap(void)
{
    ...
    breath_rap_control_data.exhale_work_time  = 2000;
