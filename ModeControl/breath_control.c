...
uint8_t check_CPAP_breath_inhale_start(void);
...
__weak void copy_control_data(ST_MAIN_CONTROL_DATA *src, ST_MAIN_CONTROL_DATA *tar)
{
    ...
    tar->T_inhale_time             = src->T_inhale_time;
...
__weak void reset_main_control_data(void)
{
    ...
    main_control_data.T_inhale_time             =  2000;
...
__weak void reset_ui_set_data(void)
{
    ...
    ui_set_data.T_inhale_time         =  2000;
...
{
    breath_cycles_save_peep_data(EM_BREATH_ADJUST_SAVE_I_CONTROL, get_peep_pwm_comtrol_data(), get_exhale_flow());
...
{
    breath_cycles_save_peep_data(EM_BREATH_ADJUST_SAVE_E_CONTROL, get_peep_pwm_comtrol_data(), get_exhale_flow());
...
{
    breath_cycles_save_peep_data(EM_BREATH_ADJUST_SAVE_T_CONTROL, get_peep_pwm_comtrol_data(), get_exhale_flow());
...
#if SIMPLE_CLIBRATION_FLOW_ENABLE
    do_simple_calibration_flow_work();
#endif
}
...
__weak void breath_cpap_mode(void)
{
    ...
    case EM_PATIENT_BREATH_DETECT: // 呼吸检测
        check_mode_change_flag();
        // 检测吸气
        if(check_CPAP_breath_inhale_start())
        {
    ...
    case EM_PATIENT_INHALE_DETECT: // 吸气检测
        ...
        if(check_CPAP_breath_inhale_start()) //	即患者出现了主动吸气的动作
        {
    ...
    case EM_PATIENT_INHALE_PERIOD: // 吸气过程
        ...
        else
        {
            ...
            if(is_rap_enable() && rap_run_flag == 0)
            {
                ...
                if(!is_rap_time_over(EM_RAP_INHALE_METHOD))
                {
                    // 如果叩击时间不够，则维持
                    break;
                }
    ...
    case EM_PATIENT_EXHALE_PERIOD: // 呼气过程
        ...
        if(is_rap_enable() && rap_run_flag == 0)
        {
            ...
            if(!is_rap_time_over(EM_RAP_EXHALE_METHOD))
            {
                // 如果叩击时间不够，则维持
                break;
            }
        ...
        if(monitor_flag == 0)
        {
            if(monitor_exhale_end())
            {
                // 告诉系统，现在开始计算PEEP
                set_peep_start_flag();
                breath_set_exhale_trigger_flag(); // 这个主动呼气的标记暂时没有用
...
__weak void reset_st_ST_mode_control_data(void)
{
    ...
    st_ST_mode_control_data.T_exhale_start_gate      = ms_1_count + 3000;
#if BREATH_ENABLE_C_FLEX
    st_ST_mode_control_data.c_flex_completed_gate    = ms_1_count + 4500;
#endif
    st_ST_mode_control_data.T_exhale_3_4_gate        = ms_1_count + 5250;// = 3000 + 3000 * 3 / 4
    st_ST_mode_control_data.T_next_breath_start_gate = ms_1_count + 6000;
}

// 启动T模式的吸气
//__weak void breath_ST_T_inhale_start(void)
//{
...
__weak void reset_ST_T_time_stemp(void)
{
    ...
    st_ST_mode_control_data.T_exhale_start_gate = ms_1_count + main_control_data.T_inhale_time;

    if(st_ST_mode_control_data.T_one_breath_time > main_control_data.T_inhale_time + 500)
    {
        st_ST_mode_control_data.T_exhale_3_4_gate = ((st_ST_mode_control_data.T_one_breath_time - main_control_data.T_inhale_time) * 3 / 4) +
                st_ST_mode_control_data.T_exhale_start_gate;
    }
    else
    {
        st_ST_mode_control_data.T_exhale_3_4_gate = st_ST_mode_control_data.T_next_breath_start_gate - 200;
    }
    //st_ST_mode_control_data.c_flex_completed_gate = st_ST_mode_control_data.T_exhale_start_gate + 300;
}
...
__weak void breath_ST_start_T_mode(void)
{
    ...
    // general a inhale action of a breath
 ...
 */
__weak void copy_stv_parameter_to_main_control_data(void)
{
    main_control_data.epap           = ui_set_data.epap;
 ...
 */
__weak void prepare_stv_control_data(void)
{
    uint32_t t1;
    uint16_t speed;
    //int16_t press;
    ...
    {
        st_ST_mode_control_data.T_exhale_3_4_gate = ((t1 - main_control_data.T_inhale_time) * 3 / 4) +
                st_ST_mode_control_data.T_exhale_start_gate;
    }
    else
    {
...
#if BREATH_ENABLE_C_FLEX
    st_ST_mode_control_data.c_flex_completed_gate = st_ST_mode_control_data.T_exhale_start_gate + (st_ST_mode_control_data.T_one_breath_time - main_control_data.T_inhale_time) / 2;
#endif
    PID_blower_control_data.base_speed           = PID_blower_control_data.Low_speed;
    set_blower_speed(PID_blower_control_data.base_speed);
...
/**
 * [is_ST_breath_exhale_start_time description]
 * @method is_ST_breath_exhale_start_time
 * @return                                [description]
 */
__weak uint8_t is_ST_breath_exhale_start_time(void)
{
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
    return system_direct_compare_tm1_be_tm2(ms_1_count, st_ST_mode_control_data.T_exhale_start_gate);
#else
    return st_ST_mode_control_data.T_exhale_start_gate < ms_1_count;// TDB
#endif
}

#if 0
__weak int8_t is_ST_T_inhale_time_finished(void)
{
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
    return system_direct_compare_tm1_be_tm2(ms_1_count, st_ST_mode_control_data.T_exhale_start_gate);
#else
 ...
 */
__weak void to_breath_stv_mode(void)
{
    ui_set_data.change_parameter_flag = 0;
...
/**
 * [ST_get_exhale_end_time description]
 * @method ST_get_exhale_end_time
 * @return                        [description]
 */
__weak uint16_t ST_get_exhale_end_time(void)
{
    return st_ST_mode_control_data.T_exhale_3_4_gate;
}
...
{
    // set flow_gate
    st_ST_mode_control_data.flow_e_gate = display_temp_data.inhale_peak_flow >> 2;
 ...
 */
__weak void breath_stv_mode(void)
{
    switch(get_patient_status())
    {
    ...
    case EM_PATIENT_INHALE_DETECT: // 吸气检测
        ...
        // 如果有主动吸气的动作，则认为吸气开始
        // ****** 如果长时间（30s）检测不到吸气或呼气，则视为呼吸停止，需要强制到EM_PATIENT_BREATH_DETECT状态中
        if(check_breath_inhale_start())//	即患者出现了主动吸气的动作
        {
            // Mark this is a trigger breath（患者有自主呼吸的过程）
            breath_set_trigger_flag();
            //display_temp_data.last_work_status = EM_PATIENT_INHALE_DETECT;
    ...
    case EM_PATIENT_EXHALE_DETECT: // 呼气检测
        ...
        if(check_breath_exhale_start())
        {
            ...
            Blower_start_flow_compensate_for_exhale();
            set_patient_status(EM_PATIENT_EXHALE_START);
            break;
        }
        // 如果时间到，则转到呼气相
        if(is_ST_breath_exhale_start_time())
        {
#if USE_BREATH_CYCLES_ADJUST
            // 这里设置IPAP
            if(breath_adjust_flag == 0)
    ...
    case EM_PATIENT_EXHALE_START: // 启动呼气
        rap_run_flag = 0;
#if USE_BREATH_CYCLES_ADJUST
    ...
    case EM_PATIENT_EXHALE_PERIOD: // 呼气的过程
#if PEEP_VALVE_CHANGE_ENABLE
        if(!peep_delay_actived_flag)
        {
            peep_delay_actived_flag = breath_waiting_peep_e_ready();
    ...
    case EM_PATIENT_INHALE_START: // 启动吸气
        rap_run_flag = 0;
#if USE_BREATH_CYCLES_ADJUST
        // 这里进行调整
...
#endif
        set_patient_status(EM_PATIENT_INHALE_PERIOD);
        break; //case EM_PATIENT_INHALE_START

    // 机械通气过程
    case EM_PATIENT_T_INHALE_START: // 启动机控的吸气
        set_breath_rap_stop();
        ...
        break; // case EM_PATIENT_T_INHALE_START:
    ...
    case EM_PATIENT_INHALE_PERIOD: // 吸气过程
        ...
        // 如果压力达到ipap 则进入 叩击/呼气检测(EM_PATIENT_EXHALE_DETECT)
...
#endif
        break; //case EM_PATIENT_INHALE_PERIOD:

    case EM_PATIENT_T_INHALE_PERIOD:// T模式吸气过程（机控吸气）
#if PEEP_VALVE_CHANGE_ENABLE
        ...
        // 如果满足条件，则直接进入呼气状态
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
        if(system_direct_compare_tm1_be_tm2(ms_1_count, st_ST_mode_control_data.T_exhale_start_gate))
#else
        if(ms_1_count >= st_ST_mode_control_data.T_exhale_start_gate)
#endif
        {
#if USE_BREATH_CYCLES_ADJUST
        ...
        if(st_ST_mode_control_data.rising_time_count > st_ST_mode_control_data.rising_time_steps)
        {
            ...
            // force!! change work status
            set_patient_status(EM_PATIENT_T_EXHALE_DETECT);

            // 恢复压力上升计数器
            st_ST_mode_control_data.rising_time_count = 0;
            break; //case EM_PATIENT_T_INHALE_PERIOD:
        }
        ...
        break; // case EM_PATIENT_T_INHALE_PERIOD:
    case EM_PATIENT_T_EXHALE_PERIOD: // T模式呼气过程
#if PEEP_VALVE_CHANGE_ENABLE
        if(!peep_delay_actived_flag)
        {
            peep_delay_actived_flag = breath_waiting_peep_e_ready();
        ...
        // 在呼气时，需要监测的内容
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
        if(!system_direct_compare_tm1_be_tm2(ms_1_count, ST_get_exhale_end_time()))
#else
        if(ms_1_count < ST_get_exhale_end_time())
#endif
        {
            // !!! 是不是错了？
            break;
...
__weak void reset_PCV_T_time_stemp(void)
{
    ...
    st_PCV_mode_control_data.T_exhale_start_gate      = ms_1_count + main_control_data.T_inhale_time;

    if(st_PCV_mode_control_data.T_one_breath_time > main_control_data.T_inhale_time + 500)
    {
        st_PCV_mode_control_data.T_exhale_3_4_gate    = ((st_PCV_mode_control_data.T_one_breath_time - main_control_data.T_inhale_time) * 3 / 4) +
                st_PCV_mode_control_data.T_exhale_start_gate;
    }
    else
    {
        st_PCV_mode_control_data.T_exhale_3_4_gate    = st_PCV_mode_control_data.T_next_breath_start_gate - 200;
    }
}
...
/**
 * [pcv_get_inhale_end_time description]
 * @method pcv_get_inhale_end_time
 * @return                          [description]
 */
__weak uint16_t pcv_get_inhale_end_time(void)
{
    return (main_control_data.T_inhale_time + main_control_data.T_inhale_time + main_control_data.T_inhale_time) >> 2;
}

/**
 * [pcv_get_exhale_end_time description]
 * @method pcv_get_exhale_end_time
 * @return                         [description]
 */
__weak uint16_t pcv_get_exhale_end_time(void)
{
    return st_PCV_mode_control_data.T_exhale_3_4_gate;
}
...
__weak void prepare_PCV_control_data(void)
{
    ...
    if(t1 > main_control_data.T_inhale_time + 500)
    {
        st_PCV_mode_control_data.T_exhale_3_4_gate    = ((t1 - main_control_data.T_inhale_time) * 3 / 4) +
                st_PCV_mode_control_data.T_exhale_start_gate;
    }
    else
    {
...
/**
 * [breath_PCV_start_inhale description]
 * @method breath_PCV_start_inhale
 */
__weak void breath_PCV_start_inhale(void)
{
    // 需要关闭叩击
    set_breath_rap_stop();
...
//{
//	// set flow_gate
//	st_PCV_mode_control_data.flow_e_gate = display_temp_data.inhale_peak_flow >> 2;
...
__weak void breath_pcv_mode(void)
{
    ...
    case EM_PATIENT_T_EXHALE_PERIOD: // T呼气过程
        ...
        // 在呼气时，需要监测的内容
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
        if(system_direct_compare_tm1_be_tm2(ms_1_count, pcv_get_exhale_end_time()))
#else
        if(ms_1_count > pcv_get_exhale_end_time())
#endif
        {
            break;
        }
    ...
    case EM_PATIENT_T_INHALE_PERIOD: // T模式吸气过程（机控吸气）
        ...
        // 如果满足条件，则直接进入呼气状态
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
        if(system_direct_compare_tm1_be_tm2(ms_1_count, st_PCV_mode_control_data.T_exhale_start_gate))
#else
        if(ms_1_count >= st_PCV_mode_control_data.T_exhale_start_gate)
#endif
        {
#if USE_BREATH_CYCLES_ADJUST
#else
...
__weak uint8_t is_cough_finished(void)
{
    ...
    case EM_COUGH_FINISH_WITH_FLOW_LIMITED: // = 2,  呼气流量阈值				0-100		   L/min
        ...
        if(get_exhale_flow() < cough_control_data.cough_finish_gate_flow_e) return EM_TRUE;
        break;
    case EM_COUGH_FINISH_WITH_TIME_LIMITED: // = 3,  时间阈值				1-20		   0.1秒
#if USE_SYSTEM_TIMER_COMPARE_FUNCTION
...
__weak void breath_cough_mode(void)
{
    ...
    case EM_PATIENT_BREATH_DETECT: // 专门用于测试的状态
        break;
    case EM_PATIENT_INHALE_DETECT: // 监测吸气触发条件
        break;
    case EM_PATIENT_INHALE_START: // 吸气开始的时刻
        break;
    case EM_PATIENT_INHALE_PERIOD: // 吸气的过程中
        break;
    case EM_PATIENT_EXHALE_DETECT: // 监测呼气触发条件
        break;
    case EM_PATIENT_BREATH_HOLD: // 屏气过程（仅容控过程可能使用）
        break;
    case EM_PATIENT_EXHALE_START: // 吸气转呼气的时刻
        break;
    case EM_PATIENT_EXHALE_PERIOD: // 呼气过程
        break;
...
void test_start_st_mode(void)
{
    ...
    ui_set_data.T_freq         =   10;
    ui_set_data.T_inhale_time = 2000; // 2s
    ui_set_data.T_rising_time  = 1000;
...
void test_start_st_mode1(void)
{
    ...
    ui_set_data.T_inhale_time = 5000; // 2s
